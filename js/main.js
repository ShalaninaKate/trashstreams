...
/* harmony import */ var _components_player__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./components/player */ "./src/js/components/player.js");
/* harmony import */ var _components_player__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_components_player__WEBPACK_IMPORTED_MODULE_2__);
...
// Проверка на наличие videoContainer
if (videoContainer) {
  ...
  function formatDuration(time) {
    ...
    if (hours === 0) {
      return `0:${minutes}:${leadingZeroFormatter.format(seconds)}`;
    } else {
...
class Tabs {
  ...
  init() {
    ...
    const i = this.options.activeIndex;
    if (this.tabsBtns[i] && this.tabsPanels[i]) {
      this.tabsBtns[i].classList.add('tabs__nav-btn--active');
      this.tabsBtns[i].removeAttribute('tabindex');
      this.tabsBtns[i].setAttribute('aria-selected', 'true');
...
if (window.innerWidth > 992) {
  document.querySelectorAll('.simplebar').forEach(el => {
    new simplebar__WEBPACK_IMPORTED_MODULE_1__["default"](el);
  });
}
...
document.querySelectorAll('.file').forEach(block => {
  const fileInput = block.querySelector('.file-input');
  const filePreview = block.querySelector('.file-preview');
